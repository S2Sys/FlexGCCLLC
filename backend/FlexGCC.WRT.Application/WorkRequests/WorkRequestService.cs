...
using FlexGCC.WRT.Application.WorkRequests.Validation;
using FlexGCC.WRT.Domain.Common;
using FlexGCC.WRT.Domain.WorkRequests;
using ContractPriority = FlexGCC.WRT.Application.WorkRequests.Contracts.WorkRequestPriority;
using ContractStatus = FlexGCC.WRT.Application.WorkRequests.Contracts.WorkRequestStatus;
using DomainPriority = FlexGCC.WRT.Domain.WorkRequests.WorkRequestPriority;
using DomainStatus = FlexGCC.WRT.Domain.WorkRequests.WorkRequestStatus;
...
public sealed class WorkRequestService
{
    ...
    public Result<PagedResult<WorkRequestDto>> List(ContractStatus? status, string? search, int page, int pageSize)
    {
        ...
        if (status is not null)
        {
            ...
            var pageResult = _repository.GetPage(domainStatus, search, safePage, safePageSize);
            var pageItems = pageResult.Items.Select(ToDto).ToArray();
            return Result<PagedResult<WorkRequestDto>>.Success(
                new PagedResult<WorkRequestDto>(pageItems, safePage, safePageSize, pageResult.TotalCount));
        }

        var result = _repository.GetPage(null, search, safePage, safePageSize);
        var allPageItems = result.Items.Select(ToDto).ToArray();

        return Result<PagedResult<WorkRequestDto>>.Success(
            new PagedResult<WorkRequestDto>(allPageItems, safePage, safePageSize, result.TotalCount));
    }
